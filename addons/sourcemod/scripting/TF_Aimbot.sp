...
{
	CreateNative("AimBot_GetState", Native_GetState);
	CreateNative("AimBot_SetState", Native_SetState);
}

public int Native_GetState(Handle plugin, int numParams)
{
	int client = GetNativeCell(1);
	AimBotType type = GetNativeCell(2);
	switch(type)
	{
		case Aim_AimType:
		{
			return g_iAimType[client];
		}
		case Aim_AutoShoot:
		{
			return g_bAutoShoot[client];
		}
		case Aim_MedicPriority:
		{
			return g_bMedicPriority[client];
		}
		case Aim_SlientAim:
		{
			return g_bSilentAim[client];
		}
		case Aim_BunnyHop:
		{
			return g_bBunnyHop[client];
		}
		case Aim_OnlyHeadshots:
		{
			return g_bHeadshots[client];
		}
		case Aim_AutoBackstab:
		{
			return g_bAutoBackstab[client];
		}
		case Aim_ForceFOV:
		{
			return g_bForceFOV[client];
		}
	}
...
public int Native_SetState(Handle plugin, int numParams)
{
	...
	switch(type)
	{
		case Aim_AimType:
		{
			g_iAimType[client] = value;
		}
		case Aim_AutoShoot:
		{
			g_bAutoShoot[client] = value > 0 ? true : false;
		}
		case Aim_MedicPriority:
		{
			g_bMedicPriority[client] = value > 0 ? true : false;
		}
		case Aim_SlientAim:
		{
			g_bSilentAim[client] = value > 0 ? true : false;
		}
		case Aim_BunnyHop:
		{
			g_bBunnyHop[client] = value > 0 ? true : false;
		}
		case Aim_OnlyHeadshots:
		{
			g_bHeadshots[client] = value > 0 ? true : false;
		}
		case Aim_AutoBackstab:
		{
			g_bAutoBackstab[client] = value > 0 ? true : false;
		}
		case Aim_ForceFOV:
		{
			g_bForceFOV[client] = value > 0 ? true : false;
		}
	}
}
...
{
	RegAdminCmd("sm_hacks2", Command_Trigger, ADMFLAG_RCON);
	...
	PrepSDKCall_SetSignature(SDKLibrary_Server, "@_ZN14CBaseAnimating10LookupBoneEPKc", 36);
	...
	StartPrepSDKCall(SDKCall_Entity);
	PrepSDKCall_SetSignature(SDKLibrary_Server, "@_ZN14CBaseAnimating15GetBonePositionEiR6VectorR6QAngle", 55);
