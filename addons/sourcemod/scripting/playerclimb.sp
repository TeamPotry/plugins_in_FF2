...
public Action:TF2_CalcIsAttackCritical(client, weapon, String:weaponname[], &bool:result)
{
	...
	if (FF2_GetBossIndex(client) != -1 && !GetConVarBool(cvarBoss))
	{
	...
	{
		if (weapon == GetPlayerWeaponSlot(client, TFWeaponSlot_Melee) && IsClassAllowed(client))
		{
...
public OnGameFrame()
{
	bool canUseBoss = GetConVarBool(cvarBoss);
	char classname[64];
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		{
			if(!canUseBoss && FF2_GetBossIndex(i) != -1)
				continue;

			int weapon = GetEntPropEnt(i, Prop_Send, "m_hActiveWeapon");
			if (IsValidEntity(weapon))
			{
				if (weapon == GetPlayerWeaponSlot(i, TFWeaponSlot_Melee) && IsClassAllowed(i))
				{
					float vecClientEyePos[3], vecClientEyeAng[3];
					GetClientEyePosition(i, vecClientEyePos);	 // Get the position of the player's eyes
					GetClientEyeAngles(i, vecClientEyeAng);	   // Get the angle the player is looking

					//Check for colliding client
					TR_TraceRayFilter(vecClientEyePos, vecClientEyeAng, MASK_PLAYERSOLID, RayType_Infinite, TraceRayDontHitSelf, i);
					...
					PrintCenterText(i, "%.1f %.1f %.1f", fNormal[0], fNormal[1], fNormal[2]);

					if (fNormal[0] >= 30.0 && fNormal[0] <= 330.0) continue;
					if (fNormal[0] <= -30.0) continue;
					...
					if (distance >= 100.0)
					{
						TE_SetupArmorRicochet(pos, vecClientEyeAng);
